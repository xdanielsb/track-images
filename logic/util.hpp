...
void toGrayI(Mat src, Mat &dst) { cvtColor(src, dst, COLOR_BGR2GRAY); }

void toHSV(Mat src, Mat &dst) { cvtColor(src, dst, COLOR_BGR2HSV); }

void diffI(Mat src1, Mat src2, Mat &dst) { absdiff(src1, src2, dst); }

void threshI(Mat src, Mat &dst, double thresh) {
  threshold(src, dst, thresh, MAX_THRESH, THRESH_BINARY);
}

void blurI(Mat src, Mat &dst) { blur(src, dst, Size(BLUR_SIZE, BLUR_SIZE)); }
...
void drawContoursI(Mat &image, InputArrayOfArrays contours, int numElements,
                   const Scalar color, v4 hierarchy = vector<Vec4i>()) {
  for (int countourId = 0; countourId < numElements; countourId++) {
    drawContours(image, contours, countourId, color, 2, 8, hierarchy, 0,
                 Point());
  }
}

void drawCircle(Mat &dst, Point p, Scalar const color = GREEN, int radius = 1,
                int thickness = 2) {
...
void displayPointsConvex(vp2 points, Mat &frame1, Scalar const color,
                         Scalar const color2, int lim) {
  vp points2;
  if (points.size() <= lim) {
    lim = points.size();
  }
